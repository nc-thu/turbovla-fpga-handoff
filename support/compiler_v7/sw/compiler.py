...
from ncw import coeff_words, ncw_words, uniform
...
PROFILES = {
    ...
    # 96 逻辑列（B8-48 定版口径：48 物理列 × Pack2；RTL 24_pcw_rtn 同宽，
    # NCW=9 字）——G3 真段过硬件仿真门用；其余参数与 full 相同
    'full96': dict(COLS=96, CTX_WORDS=131072, W_WORDS=4096, SEQ_N=2048,
                   DDR_BYTES=8388608, ZERO_SLOT=4096),
}

OP_GEMM, OP_ATTN_S, OP_HOIST, OP_COPY, OP_LOAD, OP_STORE, OP_DONE = \
    0, 1, 2, 3, 4, 5, 15
OP_SF = 14                  # 已退役（compiler v6，2026-09-09）：仅解释器读兼容
F_SYN = 198.5e6
DMA_MAX = 262128            # 0x3FFF0（16 倍数），单描述符 DMA 上限

# pcW+RTN 编码（compiler v6 统一裁决，2026-09-09，与 RTL 24_pcw_rtn 同格式）：
#   * 每个 GEMM 族描述符（op∈{0,1,2}）后挂 NCW 个系数字
#     （NCW = ceil(COLS·24/256)：COLS=108→11、96→9、12→2），平铺逐列
#     (m,s) blob——位序见 ncw.py，与 ae_gemm rq_coeff 总线逐位一致。
#     per-tensor 层广播统一 blob（语义 = 旧 per-tensor requant，逐位不变）；
#     pcW 层放 rq_pc[col0 : col0+n_loc]，n_loc 之后的列填 (1,8) 占位
#     （本 GEMM 不产出那些列，系数不参与任何输出）。
#   * 头字 bit28 = RTN 就近舍入使能（旧格式 rq_m bit15 退役）：
#     rn=1 时 y = sat8((acc·m + 2^(s-1)) >>> s)；rn=0 保持 floor。
#   * 旧 op=14 OP_SF 独立装载字退役（解释器保留读兼容跑旧 build）。
#     代价：系数字无跨 GEMM 去重，每 GEMM 都发 NCW 字（泵 2 拍/字）。
...
def desc(op=0, a_src=0, b_src=0, sm_causal=0, y_tr=0, m=0, n=0, k=0,
         ...
         dma_len=0, dma_addr=0, j0=0, rn_en=0, fmt6=0):
    ...
    v |= dma_addr << 29
    v |= (j0 & 0xFFFF) << 62
    v |= rn_en << 28           # GEMM 族 RTN 使能（bit 28 原空闲；v6 统一格式）
    v |= fmt6 << 27            # v6 格式标记（只用在 DONE 字：解释器判格式用）
    assert v < (1 << 256)
    return v
...
def est_desc_cycles(d, cols):
    ...
    if op in (0, 1, 2):
        ...
        g += ncw_words(cols) * 2              # 系数字泵（T_CFETCH/T_CLATCH 2 拍/字）
        if op == 1:
...
class Seg:
    def __init__(self, idx, P):
        ...
        self.sf_state = None      # 已退役（v6：系数随 GEMM 挂 NCW 字，无去重）
        self.coeff_idx = set()    # NCW 系数字在 descs 里的下标（非指令：
                                  # est/macs 只算真描述符，2026-09-09 修虚报）
    ...
    def emit(self, d, note, coeff=False):
        assert len(self.descs) < self.P['SEQ_N'], \
            f'{self.name}: SEQ 溢出（{len(self.descs) + 1} > {self.P["SEQ_N"]}）'
        self.descs.append((d, note))
        if coeff:
            # NCW 系数字占 SEQ 深度但不是指令：高 4 bit 是系数数据，喂给
            # est/macs 会被解成假 GEMM（前缀 build 虚报 6200 倍，09-09 修）
            self.coeff_idx.add(len(self.descs) - 1)
            return d
        op = (d >> 252) & 0xF
        m = (d >> 228) & 0xFFFF
        k = (d >> 196) & 0xFFFF
        if op in (0, 1, 2):
            # ceil16(m)=16 行瓦片数（祖传 bug 修正：原式 (ceil16(m)//16)*16
            # 把瓦片数又除了 16，m<=256 时恒为 0——24_pcw_rtn 起就错，09-09 修）
...
class Compiler:
    ...
    def _finalize(self, seg):
        seg.emit(desc(op=OP_DONE, fmt6=1),
                 'DONE（bit27=1：v6 格式标记，GEMM 后挂 NCW 系数字）')
        cyc = sum(est_desc_cycles(d, self.P['COLS']) for i, (d, _) in
                  enumerate(seg.descs) if i not in seg.coeff_idx)
        seg.est_cycles = cyc
    ...
    def _emit_gemm(self, seg, m, n, n_loc, j0, k, a_base, b_base, y_base,
                   ...
                   col0=None):
        """v6 统一格式：GEMM 头字后挂 NCW 个系数字（恒逐列语义）。
        rq_pc 给定时放该层 col0..col0+n_loc 列的系数，其余列填 (1,8)；
        否则广播 per-tensor (m,s) 到全列。RTN 走头字 bit28。
        col0 = 本 GEMM 首列在层的全局输出列里的下标——描述符 j0 是 Y 图
        散布偏移，两者在 chunk 打包 / 逐头块两种布局下不同
        （col0 缺省 = j0，全宽 Y 布局）。"""
        cols = self.P['COLS']
        rq_m, rq_s = rq[0], rq[1]
        if rq_pc is not None:
            c0 = j0 if col0 is None else col0
            slots = [tuple(x) for x in rq_pc[c0:c0 + n_loc]]
            assert len(slots) == n_loc, \
                f'{note}: rq_pc 切片 {len(slots)} != n_loc {n_loc}'
            for m_, s_ in slots:
                assert 1 <= m_ <= 32767 and S_MIN_RT <= s_ <= self.rq_max_s, \
                    f'pcW 系数非法 (m={m_},s={s_})'
            coef = slots + [(1, 8)] * (cols - n_loc)
        else:
            assert 1 <= rq_m <= 32767, f'requant m 非法 {rq_m}'
            if self.rtn:
                assert rq_s >= S_MIN_RT, f'requant s={rq_s} < RTN 下限 9'
            coef = uniform(rq_m, rq_s, cols)
        seg.emit(desc(op=op, m=m, n=n, k=k, a_base=a_base, b_base=b_base,
                      y_base=y_base, b_spad=n_loc, j0=j0, y_tr=y_tr,
                      sm_causal=sm_causal,
                      rn_en=1 if self.rtn else 0), note)
        self.stats['d_gemm'] += 1
        for w in coeff_words(coef, cols):
            seg.emit(w, f'NCW 系数 ({n_loc} 列)', coeff=True)
            self.stats['d_coeff'] += 1
    ...
    def _ph_rq(self, k):
        ...
        k<=64 时恒返回 s=8——RTN 开启时抬到 S_MIN_RT（与 apply_calib/
        _enc_r 同规矩）。抬 s 时 m 乘 2^Δs 是精确整数，数值零变化：
        sat8((2m·x + 2^s)>>>(s+1)) ≡ sat8((m·x + 2^(s-1))>>>s)（2026-09-09
        G1 真实 trace 编译崩在 _emit_gemm 的 s>=9 断言后补；旧 build 里
        59,864/82,854 条 per-tensor 描述符是 s=8+RTN，RTL 本来就跑不了）。"""
        r = 2048.0 / (k * 127.0 * 127.0)
        s = 8
        while s < 47 and int(round(r * (1 << s))) < 1:
            s += 1
        m = max(1, min(32767, int(round(r * (1 << s)))))
        if self.rtn and s < S_MIN_RT:
            m2 = m << (S_MIN_RT - s)
            assert m2 <= 32767, f'占位 requant 抬 s 溢出 m={m2}'
            m, s = m2, S_MIN_RT
            self.stats['rq_s_bumped'] += 1
        return (m, s)
    ...
    def _attn_temporal(self, seg, A):
        ...
        rq, rqpv, sig_s = self._attn_rq(A, A.q.so, A.k.so, A.v.so, kproj.sa)
        # v6 分批：每 GEMM 后挂 NCW 系数字（108 列 → 11 字），单段塞下全部
        # U×H 个 unit 会到 4705 字 >> SEQ_N=2048（2026-09-09 全量编译 seg_1049
        # 崩）。每个 unit 自包含（LOAD Q/K → COPY+GEMM 分块 → STORE 自己的
        # S 块），按批关段即可，与 swin 伪窗批同款。
        nck = max(1, ceil_div(mk, cols))
        est1 = 4 + nck * (2 + ncw_words(cols))
        seq1 = max(1, (P['SEQ_N'] - 2 - 8) // est1)
        PW = U * H
        for u0 in range(0, PW, seq1):
            nb = min(seq1, PW - u0)
            if seg.descs:
                seg = self._close(seg)
    ...
    def _attn_swin(self, seg, A):
        ...
        rq, rqpv, sig_s = self._attn_rq(A, A.qkv.so, A.qkv.so, A.qkv.so,
                                        kproj.sa)
        PW = W * H                                  # 伪窗总数
        est1 = max(1, -(-T // cols)) * (2 + ncw_words(cols)) + 2
                                                  # 每伪窗 COPY+GEMM(+NCW)+STORE
        seq1 = max(1, (P['SEQ_N'] - 2 - 8) // est1)
        ...
        for pw0 in range(0, PW, bw):
            ...
            # F4（2026-09-10 修）：sreg 单缓冲时，STORE pw 的写引擎还在读 CTX
            # 里的 S 块，下一个伪窗的 QK^T 已经把同一块 y 覆盖成新值
            # （实测 seg_0003 95/95 对全部咬上）。改成两块按伪窗奇偶交替：
            # 缓冲 (pw0+pw)%2，重写它的 GEMM 是 pw+2（隔代），中间隔着
            # STORE pw+1 的发射门 !wr_busy（结构性证明 S1a/S2/S3，两 build
            # 3,400+3,442 段 0 违反；VCD 实测 A_min=9 拍、B_min=619 拍）。
            sreg0 = seg.ctx_alloc('ss0', T16 * T)
            sreg1 = seg.ctx_alloc('ss1', T16 * T)
            assert None not in (qreg, kreg, sreg0, sreg1)
            if T % 16:
                self._prezero(seg, sreg0, T16 * T)
                self._prezero(seg, sreg1, T16 * T)
            ...
            for pw in range(nb):
                w, h = divmod(pw0 + pw, H)
                sreg = sreg0 if (pw0 + pw) % 2 == 0 else sreg1
        ...
        est2 = 2 + T16 + 4 + (2 + nwg) * (1 + ncw_words(cols)) + 2
                                     # 每伪窗 COPY/PV(+NCW)+窗尾条纹/投影(+NCW)/存
        ...
        while pw0 < PW:
            nb = min(bw2, PW - pw0)
            if pw0 + nb < PW:
                # 批界对齐窗：oreg 的各头列不能跨段（批界必须是 H 的倍数）。
                # nb<H 时抬到整窗 H。旧保护只在 nb>H 时向下取整，v6 系数字把
                # est2 撑到 132 → seq2=15 < H=24（stage-3 两块），保护失效，
                # 批界切进窗中间：前段写进 oreg 的头列随段销毁，Y 每窗缺头
                # （G12 全量对拍 32 图 mismatch 根因，2026-09-09 修）。
                # est2 把窗尾投影按伪窗计费（高估），整窗批真实字数 ~420
                # << SEQ_N=2048，Seg.emit 的 SEQ 断言兜底。
                nb = max(1, nb // H) * H
            if seg.descs:
                seg = self._close(seg)
...
def main():
    ...
    ap.add_argument('--profile', choices=['smoke', 'full', 'full96'],
                    default='full')
