...
pcW+RTN（compiler v6 统一格式，2026-09-09；与 RTL 24_pcw_rtn 同一套）：
  GEMM 族（op∈{0,1,2}）描述符占 1+NCW 个字（NCW = ceil(COLS*24/256)）。
  头字后挂逐列系数 blob（ncw.py 解码，位序 = ae_gemm rq_coeff 总线）：
      y[:, j] = sat8((acc[:, j]·m_j + rn_j) >>> s_j)
      rn_j = 2^(s_j-1) 当头字 bit28（RTN）=1 且 s_j>=9，否则 0（floor）
  per-tensor 层 = 全列同 (m,s) 的 blob，数值与旧 per-tensor 语义逐位一致。
  旧格式（op=14 OP_SF 装载 + rq_s bit7/rq_m bit15）保留只读兼容：run_segment
  看 DONE 字 bit27 判格式（编译器 v6 发 1，旧 build 全 0），旧流走旧路径。
...
from ncw import ncw_words, parse_coeff, requant_pc
...
def decode(d):
    ...
    raw_m = (d >> 104) & 0xFFFF
    f['rq_rn'] = bool(raw_m & 0x8000)     # 旧格式 bit15：RTN（读兼容）
    f['rq_m'] = raw_m & 0x7FFF
    raw_s = (d >> 96) & 0xFF
    f['rq_pc'] = bool(raw_s & 0x80)       # 旧格式 bit7：逐列系数（读兼容）
    f['rq_s'] = raw_s & 0x7F
    f['rn_en'] = (d >> 28) & 1            # v6 头字 bit28：RTN（新格式）
...
def run_segment(seq, ddr_img, P):
    ...
    sf_m = [0] * cols                  # 旧格式逐列系数（OP_SF 装载，读兼容）
    sf_s = [0] * cols
    # 格式自动判别：DONE 字 bit27=1 = v6 统一格式（GEMM 后挂 NCW 系数字）；
    # 否则 = 旧格式（op=14 OP_SF + rq 标志位，含纯 per-tensor 无 op=14 的
    # 历史构建——所以判据必须显式标记，不能靠"没有 op=14"猜）。
    # 只看段末字：DONE 恒为两版编译器的最后一个字。不能扫"第一个 op=15 字"
    # ——NCW 系数字是任意数据，高 4 bit 也可能是 15（真实 build 72/1049 段
    # 踩中，扫全流会把 v6 误判旧格式、系数字被当指令解码，2026-09-09 修）。
    ...
    while pc < n_seq:
        ...
        if op == 14:                                   # SF 系数装载（旧格式）
            ...
            if legacy:                                 # 旧格式 requant
                ...
                assert all(coef[j][0] >= 1 for j in range(nl)), \
                    f'pc={pc}: 系数字未跟（m=0）'
                Y = requant_pc(acc, coef, f['rn_en'])
            m16 = ((m + 15) // 16) * 16
