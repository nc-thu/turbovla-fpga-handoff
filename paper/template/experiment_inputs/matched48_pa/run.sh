...
for variant in base overlap read2 core2; do
 ...
 files="sim/$top.sv"; for f in $src pack2_mult_dsp.sv pack2_mult_padd.sv ae_pe_p2_pa.sv ae_pe_pp_pa.sv rq_ms_x.sv rq_v2.sv; do files="$files rtl/$f"; done
 sha256sum $files > "results/$variant/sources.sha256"
 date -Is > "results/$variant/started.txt"
 if timeout 500s verilator --binary --timing -MAKEFLAGS 'CXX=/usr/bin/g++-10 AR=/usr/bin/ar LINK=/usr/bin/g++-10' -j 4 -Wno-fatal --top-module "$top" -DVERILATOR=1 -GPCOLS_TB=48 $par --Mdir "results/$variant/obj" -o sim $files > "results/$variant/build.log" 2>&1; then
