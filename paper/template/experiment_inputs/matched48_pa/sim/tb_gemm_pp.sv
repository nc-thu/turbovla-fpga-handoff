// tb_gemm_pp.sv — ae_gemm_pp（第1/2件）引擎级位精确对拍（hw/v8）
// ----------------------------------------------------------------------------
// 与 tb_gemm_p2（hw/v5 定版）同构，差别：
//   * DUT 换 ae_gemm_pp（快照双 bank 乒乓 + 双 tile_buf 两级读出 + 双写口写链）。
//   * 行为级 CTX B 口换双写口（地址奇偶分 bank 的契约：同拍两地址不同）。
//   * WB2_TB 参数 =1 双写口 / =0 单写口（拍数台阶对拍用，同一 DUT 两种编译）。
//   * 增加 L 系列长描述符（m=64，k=5/64/2049）：行组数多，稳态周期 P 可从
//     (cycles - startup - tail)/(mt-1) 直接量出——供模型 vs RTL 对拍
//     （notes/p2_cyc.py）。深段存储加大：CTX_WORDS=16384 / W_WORDS=4096，
//     L 系列 y_base=12000 避开 A 区。
// 判定：TB_GEMM_PP ALL PASS / FAIL；黄金模型与 tb_gemm_p2 逐字一致。
`timescale 1ns/1ps
module tb_gemm_pp #(
  ...
  ae_gemm_pp #(.PCOLS(PCOLS), .PULSE_DLY(PULSE_DLY_TB), .WB2(WB2_TB)) dut (
  ...
  // ---- 行为级 CTX：A 口 1 拍回数广播；B 口双写口（同拍地址必不同）----
  ...
  // 跑一个描述符并对拍（da/dy = A 区 / Y 区基址，L 系列用大 y_base 避开 A 区）
  task run_desc(input integer dm, input integer dn, input integer dnl,
    ...
    begin
      ...
      if (err_total == 0)
        $display("[%0s] cycles=%0d mac=%0d m=%0d n=%0d nl=%0d j0=%0d k=%0d tr=%0d wb2=%0d",
                 tag, cyc_cnt, mac_cnt, dm, dn, dnl, dj0, dk, dtr, WB2_TB);
      else
