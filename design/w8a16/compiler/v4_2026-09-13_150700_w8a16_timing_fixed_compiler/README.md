...
生成时刻：2026-09-13 15:23:11（v4 timing-fixed 修订）
...
编译器内部先维护 83 条逻辑操作。向量长度超过 64-bit 指令中的 16-bit transport 字段时，`compile_program.py` 自动切成不超过 65535 个元素／beat 的连续片段，并在 JSON 中写入 `logical_id`、`logical_length`、`base_length`、`transport_length`、`chunk_index`、`chunk_count` 和完整的元素／beat 偏移。本版示例因此输出 172 条可传输 word，而不是把超长长度截断。验证器会检查所有分片首尾相接，避免漏搬或重复搬。
...
每条记录都带 `address_mode`。寄存器选择和偏移都能放入紧凑字段时使用 `compact_8bit`；只要真实张量偏移超出 8 bit，就标为 `sideband_32` 并由上层 tile scheduler 取用 32-bit 偏移。这样不会把 8-bit word 字段误当成完整地址。
...
编译器会检查 PC 连续、字编码、长度范围、矩阵尺寸、16×48 tile shape/grid、BMM 阵列选择位、扩展地址标志和长流覆盖范围。LayerNorm/Softmax 的 gamma、beta、mask 作为依赖字段保留；如果运行时没有这些 sideband，记录为 `mapped_candidate`，不会把不完整的参数路径伪装成逐位一致。
