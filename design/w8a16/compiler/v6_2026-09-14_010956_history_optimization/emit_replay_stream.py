...
def pack_descriptor(d: dict) -> int:
    ...
    # Sideband flags mirror the compiler option bits.  The top uses the low
    # eight bits for activity accounting; higher-level details remain JSON.
    opt_bits = {"pipeline": 1, "fanout": 2, "tile_mask": 4, "order": 8,
                "act_reuse": 16, "weight_reuse": 32, "overlap": 64,
                "write_coalesce": 128}
    flags = 0
    for name in d.get("optimization_flags", []):
        flags |= opt_bits.get(name, 0)
    tile = u32(c.get("tile_count")) & 0xFFFF
    # The top decodes op at [63:56] and tile count at [31:16].
    value &= ~(((0xFF) << 56) | (0xFF << 48) | (0xFFFF << 16))
    value |= (op & 0xFF) << 56
    value |= (flags & 0xFF) << 48
    value |= tile << 16
...
def main() -> None:
    ...
    for i in insts:
        ...
        if op == "END":
            ...
            flags = 0x80 if op in {"GEMM_W8A16", "BMM_W8A16", "LAYER_NORM", "GELU"} else 0
            words.append(f"0x{((OPCODE.get(op, 0x50) << 56) | ((flags & 0xFF) << 48) | ((int(i['descriptor_id']) & 0xFFFF) << 32) | (int(i['pc']) & 0xFFFF)):016x}")
