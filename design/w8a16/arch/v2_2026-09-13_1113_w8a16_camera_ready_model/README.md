# TurboVLA W8A16 架构（v2 camera-ready）
...
DMA LOAD 返回的数据会写入 runtime 的 16-lane INT16 scratch 和 48-byte INT8 weight window；STORE 在命令接收时锁存 beat，遇到 back-pressure 仍保持数据不变。超过本地窗口的模型张量必须由上层 scheduler 以多个 tile 命令搬运。
