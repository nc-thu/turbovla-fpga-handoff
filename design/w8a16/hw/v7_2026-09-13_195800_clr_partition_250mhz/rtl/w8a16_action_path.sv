...
module w8a16_action_path #(
  ...
  typedef enum logic [3:0] {S_IDLE, S_ACCUM, S_ACCUM_DRAIN1, S_ACCUM_DRAIN2,
                            S_BIAS, S_TANH, S_TANH_WAIT, S_EMIT} state_t;
  state_t state;
  ...
  // The old accumulator fed back through a variable-indexed multiplier and
  // the 40-bit adder in one cycle.  These registers make the boundary
  // explicit: select operands, multiply into mac_product_r, then add that
  // registered product on the following cycle.  A two-cycle drain commits the
  // final two products before biasing starts.
  logic signed [15:0] mac_a_r;
  logic signed [7:0]  mac_w_r;
  logic signed [31:0] mac_product_r;
  logic signed [31:0] product_comb;
  logic signed [39:0] product_ext;
  logic               mac_input_v;
  logic               mac_product_v;
  logic [N_IDX_W-1:0] mac_col_launch_r;
  logic [N_IDX_W-1:0] mac_col_r;
  ...
  always_comb begin
    // This is the only combinational multiply in the action path.  Its input
    // operands are already registered; the result is captured separately
    // from the feedback adder below.
    product_comb = $signed(mac_a_r) * $signed(mac_w_r);
    product_ext = $signed({{8{mac_product_r[31]}}, mac_product_r});
    bias_stage_next = '0;
    for (i = 0; i < N; i = i + 1)
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      case (state)
        ...
        S_ACCUM: begin
          // Commit the product launched two cycles earlier, then launch the
          // current indexed operands.  Both operations are independent
          // register-to-register paths.
          if (mac_product_v)
            acc_r[mac_col_r] <= acc_r[mac_col_r] + product_ext;
          mac_product_r <= product_comb;
          mac_product_v <= mac_input_v;
          mac_col_r <= mac_col_launch_r;
          mac_a_r <= state_r[k_r];
          mac_w_r <= weight_r[col_r][k_r];
          mac_col_launch_r <= col_r;
          mac_input_v <= 1'b1;
          if (k_r == K-1) begin
            k_r <= '0;
            if (col_r == N-1) state <= S_ACCUM_DRAIN1;
            else col_r <= col_r + 1'b1;
          end else begin
            k_r <= k_r + 1'b1;
          end
        end
        S_ACCUM_DRAIN1: begin
          // The first drain commits the penultimate product and advances the
          // registered product to the final one.
          if (mac_product_v)
            acc_r[mac_col_r] <= acc_r[mac_col_r] + product_ext;
          mac_product_r <= product_comb;
          mac_product_v <= mac_input_v;
          mac_col_r <= mac_col_launch_r;
          mac_input_v <= 1'b0;
          state <= S_ACCUM_DRAIN2;
        end
        S_ACCUM_DRAIN2: begin
          // The second drain commits the final product.  S_BIAS is entered on
          // the next clock, after this nonblocking accumulator update is
          // visible to bias_stage_next.
          if (mac_product_v)
            acc_r[mac_col_r] <= acc_r[mac_col_r] + product_ext;
          mac_product_v <= 1'b0;
          state <= S_BIAS;
