...
module w8a16_sysarr #(
  ...
  // The clear signal is consumed by every bit of every PE accumulator.  A
  // single global net therefore becomes a very high-fanout control route
  // (v6 reached more than 31k loads).  Keep the synchronous protocol intact,
  // but explicitly partition the distribution into small column groups.  The
  // generated group nets are kept so Vivado cannot collapse them back into a
  // single long route.  This is a physical-fanout fix; it adds no cycle and
  // does not change the PE arithmetic or the start/pulse priority.
  localparam int CLR_GROUP_COLS = 8;
  localparam int CLR_GROUPS = (PCOLS + CLR_GROUP_COLS - 1) / CLR_GROUP_COLS;
  (* keep = "true", dont_touch = "true", max_fanout = 64 *)
  wire [ROWS-1:0][CLR_GROUPS-1:0] clr_group;
  genvar cg_row, cg_col;
  generate
    for (cg_row = 0; cg_row < ROWS; cg_row = cg_row + 1) begin : g_clr_row
      for (cg_col = 0; cg_col < CLR_GROUPS; cg_col = cg_col + 1) begin : g_clr_col
        assign clr_group[cg_row][cg_col] = clr;
      end
    end
  endgenerate
  ...
  generate
    for (gi = 0; gi < ROWS; gi = gi + 1) begin : g_row
      for (gj = 0; gj < PCOLS; gj = gj + 1) begin : g_col
        localparam int CLR_GROUP = gj / CLR_GROUP_COLS;
        w8a16_pe #(.ACC_W(ACC_W)) u_pe (
          .clk(clk), .rst_n(rst_n), .clr(clr_group[gi][CLR_GROUP]), .pulse_in(pl_in_pe[gi][gj]),
