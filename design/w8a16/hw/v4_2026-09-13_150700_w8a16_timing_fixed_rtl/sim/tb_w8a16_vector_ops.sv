...
module tb_w8a16_vector_ops;
  ...
  task automatic run_op(input logic [3:0] code);
    begin
      ...
      @(posedge clk); #1;
      // LayerNorm and Softmax are intentionally multi-cycle in the camera
      // ready datapath.  All other vector operations still complete in one
      // registered cycle.
      start = 1'b0;
      if (code == 4'd2 || code == 4'd5 || code == 4'd6 || code == 4'd7) begin
        while (!done) @(posedge clk);
        #1;
      end else if (!out_valid || !done) begin
        $fatal(1, "vector op handshake failed op=%0d", code);
      end
    end
  endtask
