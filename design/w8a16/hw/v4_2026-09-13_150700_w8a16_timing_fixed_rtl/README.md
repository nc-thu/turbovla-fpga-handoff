...
- Vivado：system top `synth_design` 通过；785 DSP、67,448 LUT、118,149 FF、0 BRAM，WNS +0.310 ns，关键路径为动作权重寄存器到 40-bit 累加器。1×1/4×4/16×48 阵列沿用 v3 的真实综合记录，因为 v4 只改 `vector_mul`，array top 不实例化该模块。
- DSP 对应关系保持 1 PE = 1 DSP、4×4 = 16 DSP、16×48 = 768 DSP。system top 多出的 DSP 来自动作路径中的小型乘法单元。

## 不能省略的边界

仿真使用行为级乘法，服务器没有 `xvlog`，所以 UNISIM/DSP48E2 仍未跑。Vivado OOC 没有板级 I/O pin 约束，NSTD-1/UCIO-1 需要在具体板卡工程中处理；当前结果不是 bitstream-ready，也不是 post-route 频率。array 记录的来源版本和继承原因保存在 `data/v4_2026-09-13_150700/vivado_synth_results.json`。
