...
module tvla_replay_top #(
  ...
  parameter bit OPT_DOUBLE_BUFFER = 1'b1,
  parameter bit OPT_LOCAL_SCHED = 1'b1,
  parameter bit OPT_WRITE_BURST = 1'b1
) (
  input  logic clk,
  input  logic rst_n,
  ...
  wire [7:0] d_opt_flags = desc_data[55:48];
  ...
  // A completed GEMM keeps its result until the consumer accepts it.  This
  // makes the double-result-buffer promise explicit instead of emitting a
  // one-cycle pulse that can be lost under output backpressure.
  logic result_valid_r;
  logic [PCOLS*ACC_W-1:0] result_data_r;
  wire result_slot_free = (~result_valid_r) || output_ready || !OPT_DOUBLE_BUFFER;
  wire cmd_is_gemm = (cmd_op == OP_GEMM) || (cmd_op == OP_BMM);
  // A command removes one descriptor from the FIFO.  Keep this expression
  // separate from the sequential block so a descriptor push and command pop
  // in the same clock update the count exactly once.
  wire cmd_fire = cmd_valid && cmd_ready;
  wire cmd_pop = cmd_fire && !cmd_is_end;
  assign cmd_ready = (replay_done == 1'b0) &&
                     ((cmd_is_end && result_slot_free) ||
                      ((fifo_count != 0) && (!cmd_is_gemm || result_slot_free)));
  ...
  logic [63:0] optimization_hits_r, write_burst_r;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      result_valid_r <= 1'b0; result_data_r <= '0;
      ...
      if (cmd_fire) begin
        if (cmd_op == OP_END) begin
          ...
          current_valid <= 1'b1;
          if (data_fifo[fifo_head][55:48] != 8'h00)
            optimization_hits_r <= optimization_hits_r + 1'b1;
          if (OPT_WRITE_BURST && data_fifo[fifo_head][55])
            write_burst_r <= write_burst_r + 1'b1;
          ...
          if (data_fifo[fifo_head][63:56] == OP_GEMM || data_fifo[fifo_head][63:56] == OP_BMM) begin
            ...
            array_clr <= 1'b1;
            if (result_slot_free) begin
              result_valid_r <= 1'b1;
              result_data_r <= snapshot_row;
            end
      ...
      // Update FIFO occupancy once per clock.  The old implementation had
      // two nonblocking assignments here; when push and pop coincided, the
      // later assignment silently discarded the enqueue.  This case statement
      // is the actual ready/valid contract used by a streaming trace.
