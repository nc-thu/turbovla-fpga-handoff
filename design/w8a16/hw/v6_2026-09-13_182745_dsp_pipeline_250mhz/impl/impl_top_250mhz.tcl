# TurboVLA W8A16 v6: 250 MHz post-route implementation.
#
# This is an independent non-project run.  The v6 RTL adds DSP A/B input
# registers, keeps the P/output register boundary, and splits the action-path
# product from the INT40 feedback add.  No bitstream is requested because this
# wrapper has no board pin, DDR, or clock-buffer constraints.
# Usage:
#   vivado -mode batch -source impl_top_250mhz.tcl \
#     -tclargs TOP OUT ROWS PCOLS
...
if {$OUT eq ""}   { set OUT [file join [file dirname [info script]] runs default_system_250mhz] }
...
if {[llength $sources] == 0} {
  ...
  if {[catch {
    ...
    synth_design -top $TOP -part $PART -mode out_of_context \
      -flatten_hierarchy none -directive PerformanceOptimized \
    ...
    set status "passed"
    set message "250 MHz implementation and final reports completed; no bitstream requested"
  } err]} {
    set message $err
  }
}
