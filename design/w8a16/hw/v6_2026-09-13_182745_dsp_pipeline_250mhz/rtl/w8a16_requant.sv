// Pipelined INT40 to INT16 requantizer.  The multiply is registered before the
// shift/saturation stage so that the runtime top does not inherit a long
// multiplier-plus-saturation path.  The use_dsp attribute is intentional:
...
module w8a16_requant #(
  ...
  (* use_dsp = "no" *) logic signed [PW-1:0] product_r;
  logic [7:0] shift_r;
  ...
  logic v_r;
  always_comb begin
    if (shift_r >= PW) shifted = product_r[PW-1] ? -1 : 0;
    else shifted = product_r >>> shift_r;
  end
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // Stage 1: the DSP/LUT multiplier is isolated by a register.
      v_product <= in_vld;
      if (in_vld) begin
        product_r <= x * multiplier;
        shift_r <= shift;
      end

      // Stage 2: shift and signed saturation use only the registered product.
      v_r <= v_product;
