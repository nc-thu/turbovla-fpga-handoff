...
module w8a16_layernorm #(parameter int N = 16) (
  ...
  typedef enum logic [3:0] {S_IDLE, S_SUM, S_MAX, S_SCALE, S_NUM_BUILD, S_DIV, S_EMIT} state_t;
  state_t state;
    ...
    if (!rst_n) begin
      ...
      case (state)
        ...
        S_DIV: begin
          ...
          if (div_bit == 0) begin
            scaled_q = div_neg_r ? -$signed(quot_next) : $signed(quot_next);
            if (scaled_q > 32767) y[idx*16 +: 16] <= 16'sh7fff;
            else if (scaled_q < -32768) y[idx*16 +: 16] <= -16'sh8000;
            else y[idx*16 +: 16] <= scaled_q[15:0];
