"""Build the v4 timing-fixed camera-ready archive and a self-contained report.
...
DATA = ROOT / "data" / "v4_2026-09-13_150700"
COMPILER_DATA = ROOT / "compiler" / "v4_2026-09-13_150700_w8a16_timing_fixed_compiler" / "data"
RTL_ROOT = ROOT / "hw" / "v4_2026-09-13_150700_w8a16_timing_fixed_rtl"
...
def make_timing(out: Path, fig_dir: Path) -> dict[str, str]:
    labels = ["v8\nLayerNorm", "v9\niterative div", "v10\nDMA write", "v11\nfixed write", "v13\nfinal split", "v14\nscale pipeline"]
    wns = [-0.560, 0.0, -1.130, 0.0, 0.067, 0.310]
    fig, ax = plt.subplots(figsize=(7.2, 3.1))
    ax.bar(np.arange(len(wns)), wns, color=["#b05a5a", "#4f8a5b", "#b05a5a", "#2e6d49", "#2e6d49", "#2e6d49"], width=0.58)
...
def main() -> int:
    ...
    system = next((x for x in synth_records if "system_v14" in str(x.get("name"))), {})
    array = next((x for x in synth_records if "16x48" in str(x.get("name"))), {})
    st = system.get("timing", {}) or {}
    at = array.get("timing", {}) or {}
    v3_path_ns = 3.220
    v3_wns_ns = 0.067
    v3_fmax_mhz = 1000.0 / (3.298 - v3_wns_ns)
    v4_path_ns = float(st.get("data_path_delay_ns") or 0.0)
    v4_fmax_mhz = float(st.get("fmax_estimate_mhz") or 0.0)
    path_reduction_pct = (v3_path_ns - v4_path_ns) / v3_path_ns if v3_path_ns else 0.0
    fmax_gain_pct = (v4_fmax_mhz - v3_fmax_mhz) / v3_fmax_mhz if v3_fmax_mhz else 0.0

    # Machine-readable snapshot: the HTML numbers can always be traced back to
    # this file and the copied source JSON files.
    summary = {
        "schema_version": "tvla_w8a16.camera_ready_report.v4",
        ...
        "timing_comparison": {"v3_wns_ns": v3_wns_ns, "v4_wns_ns": st.get("wns_ns"), "wns_margin_gain_ns": (float(st.get("wns_ns") or 0.0) - v3_wns_ns), "v3_path_ns": v3_path_ns, "v4_path_ns": v4_path_ns, "path_reduction_fraction": path_reduction_pct, "v3_fmax_estimate_mhz": v3_fmax_mhz, "v4_fmax_estimate_mhz": v4_fmax_mhz, "fmax_gain_fraction": fmax_gain_pct},
    ...
    for r in synth_records:
        ...
        if "system_v14" in str(r.get("name")):
            label = "system top (v4)"
        else:
            label = f"array {r.get('rows')}×{r.get('pcols')}" + (" (v3 unchanged)" if r.get("source_version") else "")
...
<h1>TurboVLA W8A16 架构与编译器 camera-ready 候选</h1><div class='stamp'>生成时间：{esc(generated)}　|　版本：v4 timing-fixed　|　目录：{esc(report_dir.relative_to(ROOT))}</div>
<p class='lead'><span class='ok'>v4 的系统 top 已通过逻辑综合时序检查。</span> 本版把向量 scale 的乘法和移位／饱和拆成两级寄存器，降低了 top 的关键路径。W8A16 数值边界、编译器指令字、长向量 sideband、DMA／向量／激活单元、runtime、16×48 阵列和系统 top 都有可重复的检查。Vivado 3.298 ns 目标下 setup WNS 为 {fmt_float(st.get('wns_ns'),3,' ns')}，0 个 setup 失败端点。这里的“通过”只表示行为仿真和逻辑综合证据齐全；真实 TurboVLA 视觉／语言推理、LIBERO 环境动作、布局布线和功耗仍未完成。</p>
<div class='metrics'><div class='metric'><b>768 DSP</b><span>16×48 阵列；每个 PE 一路 A16×W8</span></div><div class='metric'><b>{fmt_float(st.get('wns_ns'),3,' ns')}</b><span>v14 综合时序余量（3.298 ns 目标）</span></div><div class='metric'><b>{logical_program} → {transport_program}</b><span>逻辑操作 → 可传输 word（长向量带完整偏移）</span></div><div class='metric'><b>{passed_tests}/{len(rtl_tests)}</b><span>收集到的 RTL 检查项为 PASS</span></div></div>
...
<h2>4. Vivado top 时序和资源</h2><p>v14 使用 Vivado 2021.2、器件 <code>xczu7ev-ffvc1156-2-e</code> 和 3.298 ns 目标时钟。系统 top 综合后的数据路径为 {fmt_float(st.get('data_path_delay_ns'),3,' ns')}，WNS 为 {fmt_float(st.get('wns_ns'),3,' ns')}，可按最差路径估算为 {fmt_float(st.get('fmax_estimate_mhz',0),1,' MHz')}。相比 v13，数据路径缩短 {fmt_pct(path_reduction_pct)}（3.220→{fmt_float(v4_path_ns,3,' ns')}），综合 Fmax 估算提高 {fmt_pct(fmax_gain_pct)}（{fmt_float(v3_fmax_mhz,1,' MHz')}→{fmt_float(v4_fmax_mhz,1,' MHz')}），WNS 绝对增加 {fmt_float(float(st.get('wns_ns') or 0.0)-v3_wns_ns,3,' ns')}。最差路径从 <code>{esc(st.get('critical_source'))}</code> 到 <code>{esc(st.get('critical_destination'))}</code>，已从 v13 的 vector scale→乘法路径转为动作路径的权重→40-bit 累加。v4 的向量 scale 已加入“乘积寄存器→移位／饱和寄存器”一级流水，LayerNorm 的迭代除法与饱和也保持分开。这个结果仍是 synth_design 估算，只有 place/route 才能确认最终布线余量。</p>{table(['配置','状态','LUT','FF','DSP','WNS','Fmax（综合估算）','数据路径','DRC 记录'],synth_table)}<p class='muted'>表中标有“v3 unchanged”的阵列行没有重新启动综合；v4 只修改 vector_mul，而 array top 不实例化它。其报告路径和原始散列仍保存在机器可读快照中。</p>
<div class='figure'><img src='figures/fig_timing_history.svg' alt='timing history' style='max-width:100%'></div><p class='muted'>图怎么看：v13 把 LayerNorm 的最后除法位和饱和拆开后，WNS 为 +0.067 ns；v14 又把向量 scale 的乘法分成两级，WNS 提升到 +0.310 ns。图中 Fmax 和 WNS 都是综合结果，不是布局布线结果。</p>
...
<h2>8. 复现入口</h2><p>在工作区运行 <code>scripts/generate_camera_ready_report_v4.py</code> 可重新生成本页；服务器 RTL 使用 <code>hw/v4_2026-09-13_150700_w8a16_timing_fixed_rtl/sim/run_verilator.sh</code> 和 <code>run_iverilog_smoke.sh</code>。Vivado 使用 <code>hw/v4_2026-09-13_150700_w8a16_timing_fixed_rtl/synth/syn_top.tcl</code>。本报告目录包含所有输入 JSON/CSV 快照、图源、<code>summary.json</code>、<code>BUILD_MANIFEST.md</code> 和修改说明。</p>
<p class='muted'>编译器程序 SHA256：{esc(compile_manifest.get('program_sha256'))}。RTL 文件散列记录在 <code>data/rtl_results.json</code>。生成时间：{esc(generated)}。</p>
    ...
    (report_dir / "MODIFICATION_NOTES.md").write_text(f"# 修改说明\n\n生成时间：{generated}\n\n本版在 v3 的动作后处理、LayerNorm/Softmax 迭代路径、DMA 数据落地、生产阵列启动和完整 sideband 基础上，给 vector_mul 增加‘乘积寄存器→移位／饱和寄存器’一级流水。v4 系统 top 使用 3.298 ns synth_design 约束，WNS 为 {st.get('wns_ns')} ns，0 个 setup 失败端点；Verilator {verilator_count} 项和 Icarus {iverilog_count} 项短 smoke 已通过。array_1x1/4x4/16x48 的资源和时序记录继承 v3，因为 array top 不实例化 vector_mul，并在机器可读结果中显式标记。报告没有把行为级乘法、综合 Fmax 或小动作 smoke 写成完整 TurboVLA/LIBERO 结果。NSTD-1/UCIO-1 仍需板级 pin 约束后才能生成 bitstream。\n", encoding="utf-8")
