...
def collect(run_root: Path) -> dict[str, Any]:
    ...
    for rec in summary.get("records", []):
        ...
        records.append(row)
    # The v4 timing edit is confined to vector_mul.  The array top does not
    # instantiate that module, so its v3 array records remain valid.  Mark
    # those records explicitly instead of silently presenting them as a new
    # synthesis invocation.
    inherited_path = ROOT / "data" / "v3_2026-09-13_140118" / "vivado_synth_results.json"
    inherited = read_json(inherited_path, {})
    existing_names = {str(r.get("name")) for r in records}
    for old in inherited.get("records", []):
        name = str(old.get("name", ""))
        if "array_" not in name or name in existing_names:
            continue
        clone = dict(old)
        clone["source_version"] = "v3_unchanged_array_top"
        clone["inheritance_note"] = "v4 vector_mul timing fix is not instantiated by top_w8a16_array"
        records.append(clone)
    result = {
        "schema_version": "tvla_w8a16.vivado_synth.v4",
        ...
        "records": records,
        "notes": [
            "system_v14 是本轮 v4 timing-fix 的真实 Vivado synth_design 结果；Fmax 由综合 WNS 按 target_period - WNS 计算，仅作估算。",
            "array_1x1/4x4/16x48 继承 v3 的真实综合记录：v4 改动只在 vector_mul，array top 不实例化该模块。",
...
def main() -> int:
    ...
    if rtl:
        ...
        rtl["limitations"] = [x for x in rtl.get("limitations", []) if "Vivado OOC" not in x and "Vivado/vivado OOC" not in x and "Vivado 逻辑综合已完成" not in x]
        rtl["limitations"].append("Vivado 逻辑综合已完成，但未运行 post-route、功耗或板级时序收敛。")
        rtl["limitations"] = list(dict.fromkeys(rtl["limitations"]))
