"""Build the v2 camera-ready archive and a self-contained Chinese HTML report.
...
DATA = ROOT / "data"
COMPILER_DATA = ROOT / "compiler" / "v2_2026-09-13_1113_w8a16_camera_ready_compiler" / "data"
RTL_ROOT = ROOT / "hw" / "v2_2026-09-13_1113_w8a16_camera_ready_rtl"
...
def make_timing(out: Path, fig_dir: Path) -> dict[str, str]:
    labels = ["v8\nLayerNorm", "v9\niterative div", "v10\nDMA write", "v11\nfixed write"]
    wns = [-0.560, 0.0, -1.130, 0.0]
    fig, ax = plt.subplots(figsize=(7.2, 3.1))
    ax.bar(np.arange(4), wns, color=["#b05a5a", "#4f8a5b", "#b05a5a", "#2e6d49"], width=0.58)
...
def main() -> int:
    ...
    system = next((x for x in synth_records if "system_v11" in str(x.get("name"))), {})
    ...
    summary = {
        "schema_version": "tvla_w8a16.camera_ready_report.v2",
        ...
        "limitations": ["DINO/T5、TurboVLA 采样循环和真实 LIBERO 环境动作尚未接入 RTL", "UNISIM/DSP48E2 smoke 因服务器没有 xvlog 未运行", "Vivado 只有 synth_design，未做 place/route、post-route、功耗或上板"],
    }
    ...
    for r in synth_records:
        ...
        label = "system top" if "system_v11" in str(r.get("name")) else f"array {r.get('rows')}×{r.get('pcols')}"
    ...
    rtl_table = [["Verilator", rtl.get("verilator", {}).get("status"), "11 tests", f"{rtl.get('verilator', {}).get('elapsed_seconds', '—')} s"], ["Icarus", rtl.get("iverilog", {}).get("status"), "5 short smoke", f"{rtl.get('iverilog', {}).get('elapsed_seconds', '—')} s"], ["UNISIM/DSP48E2", rtl.get("unisim", {}).get("status"), "xvlog unavailable", "未做"], ["system production feed", "PASS" if rtl_tests.get("tb_w8a16_system") else "FAIL", "flags[7] → 16×48 array", "Verilator"]]
    op_rows = []
...
<h1>TurboVLA W8A16 架构与编译器 camera-ready 候选</h1><div class='stamp'>生成时间：{esc(generated)}　|　版本：v2 camera-ready　|　目录：{esc(report_dir.relative_to(ROOT))}</div>
<p class='lead'><span class='ok'>当前结果可以作为 camera-ready 候选归档。</span> W8A16 数值边界、编译器指令字、DMA／向量／激活单元、runtime、16×48 阵列和系统 top 已分别通过可重复的检查。Vivado 3.298 ns 逻辑综合没有 setup 失败端点。这里的“通过”只表示行为仿真和逻辑综合证据齐全；真实 TurboVLA 视觉／语言推理、LIBERO 环境动作、布局布线和功耗仍未完成。</p>
<div class='metrics'><div class='metric'><b>768 DSP</b><span>16×48 阵列；每个 PE 一路 A16×W8</span></div><div class='metric'><b>0.010 ns</b><span>v11 综合数据路径余量（3.298 ns 目标）</span></div><div class='metric'><b>{logical_program} → {transport_program}</b><span>逻辑操作 → 可传输 word（长向量自动分片）</span></div><div class='metric'><b>{passed_tests}/{len(rtl_tests)}</b><span>收集到的 RTL 检查项为 PASS</span></div></div>
...
<h2>3. RTL 验证结果</h2><p>服务器端 Verilator 回归在 {esc(rtl.get('server_experiment'))} 完成，记录了 11 项测试。Icarus 只做秒级短 smoke，用来检查另一套 Verilog 编译器的基本行为。系统 smoke 还送入一个 <code>flags[7]=1</code> 的生产阵列命令，验证 runtime 发出 start 并等待阵列 done；阵列 feed 仍是上层 tile scheduler 的显式数据流。</p>{table(['阶段','状态','覆盖','耗时／说明'],rtl_table)}
...
<h2>4. Vivado top 时序和资源</h2><p>v11 使用 Vivado 2021.2、器件 <code>xczu7ev-ffvc1156-2-e</code> 和 3.298 ns 目标时钟。系统 top 综合后的数据路径为 {fmt_float(st.get('data_path_delay_ns'),3,' ns')}，WNS 为 {fmt_float(st.get('wns_ns'),3,' ns')}，等效时钟约 {fmt_float(st.get('clock_frequency_mhz'),1,' MHz')}。最差路径从 <code>{esc(st.get('critical_source'))}</code> 到 <code>{esc(st.get('critical_destination'))}</code>，位于 LayerNorm 归一化结果写回。它已经不是 v10 的可变权重地址写译码路径，但只有 post-route 才能确认布线后的余量。</p>{table(['配置','状态','LUT','FF','DSP','WNS','Fmax（综合估算）','数据路径','DRC 记录'],synth_table)}
<div class='figure'><img src='figures/fig_timing_history.svg' alt='timing history' style='max-width:100%'></div><p class='muted'>图怎么看：v10 的 DMA 可变目的地址曾把 WNS 拉到 -1.130 ns；v11 改为固定首／次 beat 目的地后恢复到 0 setup failure。图中 Fmax 和 WNS 都是综合结果，不是布局布线结果。</p>
...
<h2>8. 复现入口</h2><p>在工作区运行 <code>scripts/reproduce_pipeline.ps1</code> 可重建 Python、编译器、周期和 Vivado 汇总；服务器 RTL 使用 <code>hw/v2_2026-09-13_1113_w8a16_camera_ready_rtl/sim/run_verilator.sh</code> 和 <code>run_iverilog_smoke.sh</code>。本报告目录包含所有输入 JSON/CSV 快照、图源、<code>summary.json</code>、<code>BUILD_MANIFEST.md</code> 和修改说明。</p>
