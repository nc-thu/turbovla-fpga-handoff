// CTX/WRAM plus a bounded-burst AXI4 transaction controller.  The replay top
...
module tvla_memory_subsystem #(
  ...
  output logic [AXI_DW-1:0] dma_stream_out_data,
  input logic dma_stream_in_valid,
  output logic dma_stream_in_ready,
  input logic [AXI_DW-1:0] dma_stream_in_data,
  ...
  tvla_dma_axi #(.DW(AXI_DW)) u_dma (
    ...
    .stream_in_valid(dma_stream_in_valid), .stream_in_ready(stream_in_ready),
    .stream_in_data(dma_stream_in_data)
  );
  assign stream_out_ready = dma_stream_out_ready;
  assign dma_stream_out_valid = stream_out_valid;
  assign dma_stream_out_data = stream_out_data;
  assign dma_stream_in_ready = stream_in_ready;
endmodule
`endif
