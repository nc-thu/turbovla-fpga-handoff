...
def pack_descriptor_sideband(record: dict[str, Any], event: dict[str, Any]) -> int:
    ...
    # Bits 496:497 select the destination used by a DMA_READ payload router:
    # 0=CTX activation memory, 1=WRAM weight memory, 2=host/fallback only.
    # The field is deliberately in the descriptor sideband rather than the
    # 64-bit command word, so the command format stays backward compatible.
    target = str(record.get("memory_target", "ctx")).lower()
    target_code = {"ctx": 0, "wram": 1, "host": 2}.get(target, 2)
    bits = _put(bits, target_code, 496, 2)
    return bits


def memory_target_for(event: dict[str, Any], op: str) -> str:
    """Return the explicit DMA destination without guessing for other ops."""
    if op != "DMA_READ":
        return "host"
    value = event.get("memory_target", event.get("dst_space", event.get("target", "ctx")))
    value = str(value).lower()
    if value in {"ctx", "activation", "activation_memory"}:
        return "ctx"
    if value in {"wram", "weight", "weight_memory"}:
        return "wram"
    return "host"
...
def main() -> None:
    ...
    for seq, event in enumerate(events):
        ...
        record = {
            ...
            "memory_target": memory_target_for(event, op),
        }
