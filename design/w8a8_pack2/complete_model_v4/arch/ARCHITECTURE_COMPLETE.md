...
## 2026-09-16 06:56:42 v6 实施更新

本次更新补了四个容易被“接口占位”掩盖的地方：

1. 编译器的 512-bit descriptor sideband 现在按 8 个 64-bit beat 接收。地址、形状、stride、tile mask、操作码、依赖数和四个 scale/bias index 不再只停留在 JSON；`tvla_descriptor_sideband_fifo.sv` 会在命令进入 replay core 前拼好完整 sideband。
2. Pack2 顶层保留旧的 16-bit descriptor 兼容口，同时增加 sideband 口。package top 用 host kind=5 传送 sideband，因此旧 smoke 和新编译器可以共存。
3. BMM staging 同时等待 A、B 两个输入。转置、Q8.8 scale 和因果 mask 在输出行生成时处理；缺输入时保持等待，不用零数据假装完成。
4. CTX/WRAM 外围增加静态 scale/bias table。没有加载校准值时读出 FP16 1.0 和零 bias，避免未初始化数据污染 smoke；真正的校准 loader 仍需要接入完整权重流。

证据如下：Icarus 完整 top smoke 通过，sideband FIFO 逐位组包测试通过，2×2 BMM 转置/scale 测试通过。Vivado 2021.2 项目模式已能识别并综合 Xilinx Floating-Point IP（add、mul、div、exp、sqrt），说明 vendor IP 的工程引用已经修好；综合资源为 120,253 LUT、199,040 FF、822 DSP、46 BRAM，4 ns 综合 WNS=-0.411 ns。这个 WNS 仍未通过 250 MHz，而且没有做完整布线，所以不能把它写成最终时序结果。

仍未闭合的部分包括：校准值加载和真实 payload 路由、完整 LayerNorm/Softmax/GELU 的逐位数值验证、DINO/T5/action head 的层循环与权重加载、AXI burst/DDR PHY、全 trace Verilator 回放、板级实现和 LIBERO 纯整数成功率。`unknown=0` 仍只表示编译器分类完整，不表示所有算子都有专用电路。
## 2026-09-16 07:17:56 校准表加载路径

package top 的 `host_in_kind=6` 使用两拍发送一条校准记录。第一拍带 12-bit index 和三个 16-bit scale，第二拍带 32-bit bias。第二拍在 `scale_load_valid` 握手时写入 scale/bias table；descriptor 的 scale index 再从这张表读取。这样软件生成的 scale ID 不再只能落到默认值。

当前仍有两个边界：真实 checkpoint 的 scale/bias 数值还没有随 payload 送入 RTL，权重和 activation payload 也没有接入完整 DDR 回放。因此这条路径已在 RTL 和小模块仿真中存在，但不能写成“全模型已经用真实校准值执行”。
