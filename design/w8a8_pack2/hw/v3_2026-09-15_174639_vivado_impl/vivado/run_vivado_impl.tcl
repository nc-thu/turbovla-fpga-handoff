...
set top tvla_w8a8_pack2_impl_top
...
set rc [catch {
  ...
  set srcs [list \
    ...
    [file join $rtl tvla_w8a8_pack2_impl_top.sv]]
  read_verilog -sv $srcs
  set_property top $top [current_fileset]
  update_compile_order -fileset sources_1
  # A project has no open design immediately after read_verilog, so create_clock
  # cannot be issued directly here.  Add an XDC constraint instead; Vivado
  # applies it during synthesis and implementation after elaboration.
  # Keep the add_files argument relative to the project directory.  Vivado
  # 2021.2 has a Windows path parser that can split an absolute path at the
  # space in the workspace name.
  set xdc_file [file join $project_dir clock.xdc]
  set xdc_fh [open $xdc_file w]
  puts $xdc_fh [format {create_clock -name clk -period %s [get_ports clk]} $period_ns]
  puts $xdc_fh {set_clock_uncertainty 0.05 [get_clocks clk]}
  close $xdc_fh
  set old_pwd [pwd]
  cd $project_dir
  add_files -fileset constrs_1 clock.xdc
  set_property file_type XDC [get_files clock.xdc]
  set_property USED_IN {synthesis implementation} [get_files clock.xdc]
  cd $old_pwd
