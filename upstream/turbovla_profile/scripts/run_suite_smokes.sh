...
for suite in libero_object libero_goal libero_10; do
  echo "[suite-smoke] $suite start $(date -Is)"
  python "$ROOT/scripts/run_eval_patched.py" \
    ...
    --task_ids 0 --num_trials_per_task 1 \
    --num_open_loop_steps 12 --chunk_size 12 --seed 7 --precision bf16 \
    --result_json_path "$ROOT/eval_${suite}_bf16.json" \
    --log_path "$ROOT/eval_${suite}_bf16.log"
  rc=$?
  echo "[suite-smoke] $suite end $(date -Is) rc=$rc"
done
