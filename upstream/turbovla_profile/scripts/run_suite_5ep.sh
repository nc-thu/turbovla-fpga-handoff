...
for suite in libero_spatial libero_object libero_goal libero_10; do
  echo "[suite-5ep] $suite start $(date -Is)"
  python "$ROOT/scripts/run_eval_patched.py" \
    ...
    --result_json_path "$ROOT/eval_${suite}_bf16_5ep.json" \
    --log_path "$ROOT/eval_${suite}_bf16_5ep.log"
  rc=$?
  echo "[suite-5ep] $suite end $(date -Is) rc=$rc"
done
